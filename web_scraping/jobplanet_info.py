...
for page in range(end_page + 1):
    ...
    for i in range(len(data['data']['search_result']['jobs'])):
        ...
        try:
            ...
            print('no required_skills')
        job_url = data['data']['search_result']['jobs'][i]['jd']['url']

        next_response = requests.get(f'https://www.jobplanet.co.kr/{job_url}')
        # next_response = requests.get(f'https://www.jobplanet.co.kr/companies/198924/job_postings/1189855/테크사업부-백엔드-개발자/매드업?_rs_act=search&_rs_con=job%2Fsearch&_rs_element=search_result')


        soup = BeautifulSoup(next_response.text, 'html.parser')
        # job_info = soup.select('.title')
        # print('-----주요업무------------')
        # print(job_info[1].get_text(separator="\n").strip())
        # print('-----자격요건------------')
        # # print(job_info[2].select_one('.text').get_text(separator="\n").strip())
        # print(job_info[2].get_text(separator="\n").strip())
        # print('-----우대사항------------')
        # print(job_info[3].get_text(separator="\n").strip())
        try:
            main_task_info = soup.find('h3', string='주요업무').find_next_sibling('div')
            print('-----주요업무------------')
            print(main_task_info.get_text(separator='\n').strip())
            requirements_info = soup.find('h3', string='자격요건').find_next_sibling('div').find('span')
            print('-----자격요건------------')
            print(requirements_info.get_text(separator='\n').strip())
            prefer_info = soup.find('h3', string='우대사항').find_next_sibling('div')
            print('-----우대사항------------')
            print(prefer_info.get_text(separator='\n').strip())
        except:
            print('fghjdkghfjkdsghfsjkghfdsjkghfdjksghfdsjlghfdsjlghfdjskghfdsjkfdhsjklgdsfhjk')
